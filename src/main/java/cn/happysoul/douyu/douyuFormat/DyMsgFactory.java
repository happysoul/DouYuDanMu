package cn.happysoul.douyu.douyuFormat;
...
import cn.happysoul.douyu.exceptions.CheckException;
...
public class DyMsgFactory {
	 ...
	 * 
	 * @param recvBytes:
	 *            the bytes array to parse
	 * @param recvLen:
	 *            the length of valid bytes
	 * @param mappers:
	 *            A collection to collect the parsing result
	 * @return: the bytes in the end of the valid bytes, which cannot be parse
	 *          this time
	 */
	public static byte[] parseRecvMsg(byte[] recvBytes, int recvLen, Collection<MsgMapper> mappers) {
		...
		if (leftBytes == null) {
			...
			if (leftBytes.length < 12) {
				// if it less than 12 bytes, maybe it's a incomplete head, keep
				// it.
				return leftBytes;
			}
			try {
				// if it pass the check of head, it contains a good head, keep
				// it
				...
				// other wise, it does not contain a good head, try to parse it
				// carelessly
	...
	/**
	 * This method will be called only when "type@=" found in an incomplete
	 * bytes array also with an incomplete head, it means we cannot get the
	 * length of the message. carelessParse will only parse one "type@=". Parse
	 * the bytes by searching for "type@=", from tail to head, if there are
	 * still some bytes before the first "type@=", report it then ignore it.
	 */
	...
	private static byte[] carefulParse(byte[] recvBytes, int recvLen, Collection<String> dataStrs) {
		try {
			...
			// if msgLength < recvLen means the recvBytes is not complete, can
			// not continue to parse the data
			...
			// After parsing one message ahead, if there are still some bytes
			// left.
	...
	private static byte[] getByte(String data) {
		...
		try {
			...
			doutput.write(Formating.toLH(data.length() + 8), 0, 4);
			// 4 bytes packet length
			doutput.write(Formating.toLH(data.length() + 8), 0, 4);
			// 2 bytes message type
			doutput.write(Formating.toLH(DY_MESSAGE_TYPE_CLIENT), 0, 2);
			// 1 bytes encrypt
			doutput.writeByte(0);
			// 1 bytes reserve
			doutput.writeByte(0);
